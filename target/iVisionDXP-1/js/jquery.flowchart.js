...
// "colorize" the widget name
    var linkColor;
    if ($("#ssUserThemeName").val() == "-Light") {
        linkColor = '#40875e';
    } else if ($("#ssUserThemeName").val() == "-Dark") {
        linkColor = '#dedede';
    } else {
        linkColor = '#0071C5';
    }
...
//            defaultLinkColor: '#3366ff',
            ...
            onLinkMouseOver: function (linkId, linkData) {
                return true;
            },
            onLinkMouseOut: function (linkId, linkData) {
                return true;
            },
        ...
        _create: function () {
            ...
            shape.setAttribute("stroke", linkColor);
        ...
        _initEvents: function () {
            ...
            this.objs.layers.operators.on('click', '.flowchart-operator', function (e) {
                if ($(e.target).closest('.flowchart-operator-connector').length == 0) {
                    var opId = $(this).data('operator_id');
                    self.selectOperator($(this).data('operator_id'), e);
                }
            });
            this.objs.layers.operators.on('contextmenu', '.flowchart-operator', function (e) {
                if ($(e.target).closest('.flowchart-operator-connector').length == 0) {
                    var opId = $(this).data('operator_id');
//                    self.selectOperator($(this).data('operator_id'), e);
                    console.log("flowchart-operator:::contextmenu");
                    self.operatorContext($(this).data('operator_id'), e);
                }
            ...
            this.objs.layers.operators.on('click', '.flowchart-operator-connector', function () {
                ...
                if (self.options.canUserEditLinks) {
                    self._connectorClicked($this.closest('.flowchart-operator').data('operator_id'),
                            $this.data('connector'), $this.data('sub_connector'),
            ...
            this.objs.layers.links.on('dblclick', '.flowchart-link', function () {
                self._connecterDoubleClick($(this).data('link_id'));
            });
...
//                var startx = bezierFromX;
//                var endX = bezierToX;
                var xdistanceCenter = (Math.abs(bezierFromX - bezierToX) / 2) + Math.min(bezierFromX, bezierToX)

                toY = toY + 5;
                fromY = fromY + 5;
//                linkData.internal.els.path.setAttribute("d", 'M' + bezierFromX + ',' + (fromY) + ' L' + xdistanceCenter + ',' + fromY + ' ' + xdistanceCenter + ',' + toY + ' ' + bezierToX + ',' + toY);

                var xLenChange = 0;
                var yLenChange = 0;

                if (Math.abs(fromY - toY) > 10) {
                    if (Math.abs(bezierFromX - xdistanceCenter) > 10) {
                        if (bezierFromX < xdistanceCenter) {
                            xLenChange = -10;
                        } else {
                            xLenChange = 10;
                        }
                        ...
                        if (bezierFromX < xdistanceCenter) {
                            xLenChange = bezierFromX - xdistanceCenter;
                        } else {
                            xLenChange = bezierFromX - xdistanceCenter;
                        }
                    }
                    ...
                    if (Math.abs(bezierFromX - xdistanceCenter) > 5) {
                        if (bezierFromX < xdistanceCenter) {
                            xLenChange = -5;
                        } else {
                            xLenChange = 5;
                ...
                linkData.internal.els.path.setAttribute("d", 'M' + bezierFromX + ',' + (fromY) +
                        ' L' + (xdistanceCenter + xLenChange) + ',' + fromY + ' ' +
                        ' C' + (xdistanceCenter + xLenChange) + ',' + fromY + ' ' +
                        xdistanceCenter + ',' + fromY + ' ' +
                        xdistanceCenter + ',' + (fromY + yLenChange) + ' ' +
                        ' L' + xdistanceCenter + ',' + (toY - yLenChange) + ' ' +
                        ' C' + xdistanceCenter + ',' + (toY - yLenChange) + ' ' +
                        +xdistanceCenter + ',' + toY + ' ' +
                        +(xdistanceCenter - xLenChange) + ',' + toY + ' ' +
                        ' L' + bezierToX + ',' + toY);
...
//                            height: 'auto'
        ...
        selectOperator: function (operatorId, event) {
        ...
        operatorContext: function (operatorId, event) {
            if (!this.callbackEvent('operatorContextMenu', [operatorId])) {
//            if (!this.callbackEvent('operatorContextMenu', [operatorId])) {
                return;
            }
            this.unselectLink();
...
//              linkData.internal.els.path.parent.attr("title","test");
            if (this.options.verticalConnection) {
        ...
        _connecterMouseOver: function (linkId) {
            var linkData = this.data.links[linkId];
            if (!this.callbackEvent('linkMouseOver', [linkId, linkData])) {
                return;
            }
        ...
        _connecterMouseOut: function (linkId) {
            var linkData = this.data.links[linkId];
            if (!this.callbackEvent('linkMouseOut', [linkId, linkData])) {
                return;
            }
            if (this.selectedLinkId != linkId) {
                this.uncolorizeLink(linkId);
        ...
        _connecterDoubleClick: function (linkId) {
            var linkData = this.data.links[linkId];
            if (!this.callbackEvent('linkDoubleClick', [linkId, linkData])) {
                return;
            }
        ...
        getMapOperatorData: function (selectedMapOperatorId) {
            var data = {};
            var toOpLinkArray = this.getLinksFrom(selectedMapOperatorId);
            var fromOpLinkArray = this.getLinksTo(selectedMapOperatorId);
            var toOpArray = [];
            var fromOpArray = [];
            if (toOpLinkArray != null && toOpLinkArray.length != 0) {
                for (var i = 0; i < toOpLinkArray.length; i++) {
                    var toOpLink = toOpLinkArray[i];
                    if (toOpLink != null) {
                        var operatorData = this.getOperatorData(toOpLink['toOperator']);
                        if (operatorData != null) {
                            toOpArray.push(operatorData);
                        }
                    }
            ...
            if (fromOpLinkArray != null && fromOpLinkArray.length != 0) {
                for (var i = 0; i < fromOpLinkArray.length; i++) {
                    var fromOpLink = fromOpLinkArray[i];
                    if (fromOpLink != null) {
                        var operatorData = this.getOperatorData(fromOpLink['fromOperator']);
                        if (operatorData != null) {
                            fromOpArray.push(operatorData);
                        }
                    }

                }
            }
            data['toOpArray'] = toOpArray;
            data['fromOpArray'] = fromOpArray;
            return data;
        },
        getFromOperatorsByToOpId: function (selectedToOperatorId) {
            var data = {};
            var fromOpLinkArray = this.getLinksTo(selectedToOperatorId);
            var toOpArray = [];
            var fromOpArray = [];
            var fromMapOpLink = fromOpLinkArray[0];
            var mapOpId = fromMapOpLink['fromOperator'];
            fromOpLinkArray = this.getLinksTo(mapOpId);
            if (fromOpLinkArray != null && fromOpLinkArray.length != 0) {
                for (var i = 0; i < fromOpLinkArray.length; i++) {
                    var fromOpLink = fromOpLinkArray[i];
                    if (fromOpLink != null) {
        ...
        getToOperatorsByOpId: function (selectedToOperatorId) {
            var data = {};
            var toOpLinkArray = this.getLinksFrom(selectedToOperatorId);

            var toOpArray = [];
            var toMapOpLink = toOpLinkArray[0];
            var mapOpId = toMapOpLink['fromOperator'];
            toOpLinkArray = this.getLinksFrom(mapOpId);
            if (toOpLinkArray != null && toOpLinkArray.length != 0) {
                for (var i = 0; i < toOpLinkArray.length; i++) {
                    var toOpLink = toOpLinkArray[i];
                    if (toOpLink != null) {
                        var operatorData = this.getOperatorData(toOpLink['fromOperator']);
                        if (operatorData != null) {
                            toOpArray.push(operatorData);
                        }
                    }
        ...
        getAllFromOperatorsByToOpId: function (selectedToOperatorId) {
            var data = {};
            var fromOpLinkArray = this.getLinksTo(selectedToOperatorId);
            var toOpArray = [];
            var fromOpArray = [];
        ...
        getAllToOperatorsByToOpId: function (selectedFromOperatorId) {
            var data = {};
            var toOpLinkArray = this.getLinksFrom(selectedFromOperatorId);
            var toOpArray = [];
            var fromOpArray = [];
            if (toOpLinkArray != null && toOpLinkArray.length != 0) {
                for (var i = 0; i < toOpLinkArray.length; i++) {
        ...
        getAllConnToOperatorsByOpId: function (selectedToOperatorId, connToOpArray) {

            var toOpArray = this.getAllToOperatorsByToOpId(selectedToOperatorId);
            var $this = this;
            $.each(toOpArray, function (i) {
                connToOpArray.push(this);
                $this.getAllConnToOperatorsByOpId(parseInt(this['operatorId']), connToOpArray);
            });

            return connToOpArray;
        },
        getAllConnFromOperatorsByOpId: function (selectedToOperatorId, connFromOpArray) {

            var fromOpArray = this.getAllFromOperatorsByToOpId(selectedToOperatorId);
            var $this = this;
            $.each(fromOpArray, function (i) {
                connFromOpArray.push(this);
                $this.getAllConnFromOperatorsByOpId(parseInt(this['operatorId']), connFromOpArray);
            });

            return connFromOpArray;
        },
        getAllConnectedOperatorsToOp: function (selectedToOperatorId) {
            var operatorAray = [];
            var fromOpArray = this.getAllConnFromOperatorsByOpId(selectedToOperatorId, []);
            var toOpArray = this.getAllConnToOperatorsByOpId(selectedToOperatorId, []);

            $.each(fromOpArray, function (i) {
                operatorAray.push(this);
            })

            $.each(toOpArray, function (i) {
                operatorAray.push(this);
            })
            var operatorData = this.getOperatorData(selectedToOperatorId);

            operatorAray.push(operatorData);
        ...
        getLinksFrom: function (operatorId) {
            ...
            for (var linkId in this.data.links) {
                if (this.data.links.hasOwnProperty(linkId)) {
                    var linkData = this.data.links[linkId];
                    if (parseInt(linkData.fromOperator) === parseInt(operatorId)) {
        ...
        getLinksTo: function (operatorId) {
            ...
            for (var linkId in this.data.links) {
                if (this.data.links.hasOwnProperty(linkId)) {
                    var linkData = this.data.links[linkId];
                    if (parseInt(linkData.toOperator) === parseInt(operatorId)) {
                        result.push(linkData);
                    }
                }
            }
