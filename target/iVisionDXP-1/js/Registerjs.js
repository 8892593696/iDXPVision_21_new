...
//    //alert("date.getDate()::"+currentDate); // 16
    ...
    var currentYear = yr;
    var list_year = "<option value='1980'>1980</option>";
                ...
                if (result == 'false')
                {
                    ...
                    $.ajax({
                        ...
                        data: {
                            ...
                            registerDialogFlag: "Y",
                            price: $("#priceId").val(),
                            companyCode: $("#companyId").val(),
                            hrAffliateVal: $("#hrAffilatesInputId").val(),
                        },
                        cache: false,
                        success: function (result) {
                            stopLoader();
...
//       $("#divDOM").remove();
//        
//        $("<div/>", {'id': 'divDOM'}).html(UsrErrRslt + "In Personal Data").dialog({ resizable: false,
//            'title': 'Error ',
//            modal: true,
//            height: 120,
//            minWidth: 200,
...
//   //alert("hi");
    ...
    var charPos = Trim_captch && Trim_captch.charAt(4);
...
$("#dateofbirth").datepicker({
    ...
    dateFormat: "dd-mm-yy",
    showOn: "button",
    buttonImage: 'pagepics/images/iDXPUI5Calendar.svg',
...
function uemail(ele) {
    ...
    var res = patt.test(str);
    if (ele !== null && $("#" + ele).data("regex")) {
        let pattern = $("#" + ele).data("regex");
        let patt = new RegExp(pattern);
        res = patt.test(str);
        if (res && str !== null && str !== "") {
            $("#user_name").val(str.replace(/@.*/, ""));
        }
    }
    if (str && res == false)
    {
...
function showTerms(termid)
{

    console.log($("#" + termid).prop('checked'));

    if ($("#" + termid).prop('checked')) {
        console.log('checked terms and conditions');
        $("#dialog").html("<div style='display:block;margin-left: 5%;'><div style='float:left;display: inline-block;'><img height='30px' src='images/help.png'></img></div><div style='width:71%;display:inline-block;float:left;margin-top:5px;margin-left: 10%;'>Do you agree the terms and conditions?</div></div>");
        $("#dialog").dialog({
            title: `${labelobj['Terms & Conditions'] != null ? labelobj['Terms & Conditions'] : 'Terms & Conditions'}`,
            modal: true,
            width: '25%',
            height: 'auto',
            minHeight: 'auto',
            ...
            open: function () {
                ...
                $(this).closest(".ui-dialog").addClass("visionRegisterTermesDialog");
                $(".visionHeaderMain").css("z-index", "999");
                $(".visionFooterMain").css("z-index", "999");
            },
            beforeClose: function (event, ui)
...
function getNewUserRegisterForm() {
    showLoader();
    var csrfToken = $('meta[name="_csrf"]').attr('content');
    $.ajax({
        type: "POST",
        url: 'userNewRegisterForm',
        dataType: "html",
        data: {
        ...
        success: function (response) {
            ...
            $("#dialog1").dialog({resizable: false,
                modal: true,
                title: (labelObject['User Register Form'] != null ? labelObject['User Register Form'] : 'User Register Form'),
                width: '800',
                height: 'auto',
                dialogClass: 'UserRegisterFormDailogClass',
                buttons: [{
